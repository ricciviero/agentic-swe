#!/usr/bin/env python3
"""Verify that BehaviorBench v3 arms differ only by treatment."""
...
EXPERIMENT = Path("benchmarks/experiments/behaviorbench-dsv4p-20260718-08")
...
def main() -> None:
    ...
    if len(arms) != 2 or [arm.get("name") for arm in arms] != ["legacy", "authoritative"]:
        raise SystemExit("BehaviorBench v3 requires ordered legacy and authoritative arms")
    ...
    if arms_config["model"] != f"{lock['model']['provider']}/{lock['model']['id']}":
        raise SystemExit("Model differs between v3 arms and lock")
    if arms_config["reasoningEffort"] != lock["model"]["reasoningEffort"]:
        raise SystemExit("Reasoning differs between v3 arms and lock")
    shared = {
    ...
    if arms_config["concurrency"] != 1:
        raise SystemExit("Paid BehaviorBench v3 trials must remain serial")
    print("BehaviorBench v3 fairness check passed: treatment is the only arm difference.")
